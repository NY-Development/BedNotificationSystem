...
const Screenshot = () => {
	...
	}, [file, isRedirecting]); // Added isRedirecting dependency
	...
	const handleUpload = async (e) => {
		...
		try {
			...
			// 2. Clear file and preview state (not strictly necessary but good practice)
			setFile(null);
			setPreview(null);
			if (fileInputRef.current) fileInputRef.current.value = "";

			// 3. Redirect after 5 seconds
			setTimeout(() => {
				// 4. Remove the plan and perform the final navigation
				localStorage.removeItem("selectedPlan");
				setShowModal(false);
				navigate("/login");
				// The isRedirecting state will be cleared when the component unmounts on navigation
			}, 5000);
		} catch (err) {
			...
			// Do NOT set loading to false here, as it will happen on success before the timeout finishes
			if (!showModal) {
				setLoading(false);
			}
		}
